...
var ProjectDirectoryFinder = /*#__PURE__*/function () {
  function ProjectDirectoryFinder(_ref) {
    ...
    this.path = path;
    this.dirname = __dirname; // NOTICE: Order does matter as packages install via yarn 2 Plug n Play mode also
    //         include node_modules in path.

    this.PATHS_ROOT_PACKAGES = [// Yarn 2 Plug n Play mode
    this.path.join('.yarn', 'cache'), this.path.join('.yarn', 'unplugged'), // Usual Yarn / NPM
    'node_modules'];
  }
  ...
  (0, _createClass2["default"])(ProjectDirectoryFinder, [{
    ...
    value: function getAbsolutePath() {
      for (var index = 0; index <= this.PATHS_ROOT_PACKAGES.length; index += 1) {
        var rootPackagesPath = this.PATHS_ROOT_PACKAGES[index]; // NOTICE: forest-express has not been sym linked.
