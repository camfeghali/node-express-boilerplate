...
function SearchBuilder(model, opts, params, fieldNamesRequested) {
  ...
  function lowerIfNecessary(entry) {
    ...
    if (Database.isMSSQL(model.sequelize)) {
      return entry;
    }
  ...
  this.performWithSmartFields = function (associationName) {
    ...
    const query = {
      include: [],
      where: {
        [OPERATORS.AND]: [_this.perform(associationName)]
      }
    };
    ...
    return {
      ...
      conditions: query.where[OPERATORS.AND][0][OPERATORS.OR].length ? query.where[OPERATORS.AND][0] : null
    };
  };
  ...
  this.perform = function (associationName) {
    ...
    _.each(fields, function (field) {
      ...
      if (field.field === schema.idField) {
        ...
        const conditionValue = getConditionValueForNumber(params.search, model.rawAttributes[field.field].type);
