...
class PrimaryKeysManager {
  constructor(model) {
    this._primaryKeys = _lodash.default.keys(model.primaryKeys);
  ...
  annotateRecords(records) {
    ...
    if (this._primaryKeys.length > 1) {
      records.forEach(function (record) {
        record.forestCompositePrimary = _this._createCompositePrimary(record);
      });
    }
  }
  ...
  _getRecordsConditionComposite(recordIds) {
    ...
    const Ops = _operators.default.getInstance({
      Sequelize: this._Sequelize
    });

    return recordIds.length === 1 ? this._getRecordConditions(recordIds[0]) : {
      [Ops.OR]: recordIds.map(function (id) {
        return _this2._getRecordConditions(id);
      })
    };
  }
  ...
  _getRecordConditions(recordId) {
    return _lodash.default.zipObject(this._primaryKeys, this._getPrimaryKeyValues(recordId));
  }
  /** Create packed recordId from record */


  _createCompositePrimary(record) {
    return this._primaryKeys.map(function (field) {
      return record[field] === null ? 'null' : record[field];
  ...
  _getPrimaryKeyValues(recordId) {
    ...
    const unpacked = recordId.split(PrimaryKeysManager._GLUE).map(function (key) {
...
(0, _defineProperty2.default)(PrimaryKeysManager, "_GLUE", '|');
