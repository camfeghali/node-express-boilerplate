...
class ResourceCreator {
  ...
  async _makePromisesBeforeSave(record, [name, association]) {
    if (association.associationType === 'BelongsTo') {
      ...
      return record[setterName](targetKey, {
        save: false
      });
    }
  ...
  async _handleSave(record, callback) {
    ...
    if (associations) {
      await P.all(Object.entries(associations).map(function (entry) {
        return callback.bind(_this)(record, entry);
      }));
    }
  }
  ...
  async perform() {
    // buildInstance
    const recordCreated = this.model.build(this.params); // handleAssociationsBeforeSave

    await this._handleSave(recordCreated, this._makePromisesBeforeSave); // saveInstance (validate then save)
    ...
    const record = await recordCreated.save(); // handleAssociationsAfterSave
    // NOTICE: Many to many associations have to be set after the record creation in order to
    //         have an id.

    await this._handleSave(record, this._makePromisesAfterSave); // appendCompositePrimary

    new PrimaryKeysManager(this.model).annotateRecords([record]); // return makeResourceGetter()
