...
module.exports = function (model, opts) {
  ...
  function getInverseOf(association) {
    const remoteAssociation = Object.values(association.target.associations).find(function (a) {
      return a.identifierField === association.identifierField;
    });
  ...
  return P.all([columns, associations]).then(function () {
    ...
    Object.entries(model.associations).forEach(function ([, association]) {
      const primaryKeyIsAForeignKey = isPrimaryKeyAForeignKey(association);

      if (primaryKeyIsAForeignKey) {
        const FieldWithForeignKey = fields.find(function (field) {
          return field.reference === `${association.target.name}.${association.target.primaryKeyField}`;
        });
    ...
    _.remove(fields, function (field) {
      return _.includes(fieldNamesToExclude, field.columnName) && !field.isPrimaryKey;
    });
